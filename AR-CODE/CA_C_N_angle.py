...
def evaluate(eval_model, CA_C_N_angle_valid_loader):
    eval_model.eval()  # Turn on the evaluation mode
    total_loss = 0.

    with torch.no_grad():
        for enc_inputs, dec_inputs, dec_outputs in CA_C_N_angle_valid_loader:
            enc_inputs, dec_inputs, dec_outputs = enc_inputs.to(device), dec_inputs.to(device), dec_outputs.to(device)
            outputs, enc_self_attns, dec_self_attns, dec_enc_attns = model(enc_inputs, dec_inputs)
    ...
    return total_loss / len(CA_C_N_angle_valid_loader)
...
def test(data_loader,path):
    ...
    for i in range(0, len(b)):
        output_file_path = '../result/CA_C_N_angle_pred/'
...
# path_checkpoint = "../model/CA_C_N_angle_model/ckpt_best.pth"
...
if __name__ == "__main__":
    ...
    # ----------------------------------------tensorboard----------------------------------------
    loss_show = []
    train_writer = SummaryWriter('../result/CA_C_N_angle_plot/loss/train')
    val_writer = SummaryWriter('../result/CA_C_N_angle_plot/loss/val')
    # ----------------------------------------tensorboard----------------------------------------
    ...
    for epoch in range(start_epoch + 1, EPOCH+1):
        ...
        if not os.path.isdir("../model/CA_C_N_angle_model"):
            os.mkdir("../model/CA_C_N_angle_model")
        # torch.save(checkpoint, '../model/CA_C_N_angle_model/ckpt_%s.pth' % (str(epoch)))
        lr_scheduler.step()

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_model = model
            torch.save(checkpoint, '../model/CA_C_N_angle_model/ckpt_best.pth')
    ...
    print('----------PREDICTION-----------')
    test(test_loader,'../model/CA_C_N_angle_model/ckpt_best.pth')
    # test(test_loader1, '../model/CA_C_N_angle_model/ckpt_best.pth')

...
from Data import train_loader,valid_loader,test_loader,rest,max,min,lendict
...
def evaluate(eval_model, omega_valid_loader):
    eval_model.eval()  # Turn on the evaluation mode
    total_loss = 0.

    with torch.no_grad():
        for enc_inputs, dec_inputs, dec_outputs in omega_valid_loader:
            enc_inputs, dec_inputs, dec_outputs = enc_inputs.to(device), dec_inputs.to(device), dec_outputs.to(device)
            outputs, enc_self_attns, dec_self_attns, dec_enc_attns = model(enc_inputs, dec_inputs)
    ...
    return total_loss / len(omega_valid_loader)
...
def test(data_loader,path):
    ...
    for i in range(0, len(b)):
        output_file_path = '../result/omega_pred/'
...
# path_checkpoint = "../model/omega_model/ckpt_best.pth"
...
if __name__ == "__main__":
    ...
    # ----------------------------------------tensorboard----------------------------------------
    loss_show = []
    train_writer = SummaryWriter('../result/omega_plot/loss/train')
    val_writer = SummaryWriter('../result/omega_plot/loss/val')
    # ----------------------------------------tensorboard----------------------------------------
    ...
    for epoch in range(start_epoch + 1, EPOCH+1):
        ...
        if not os.path.isdir("../model/omega_model"):
            os.mkdir("../model/omega_model")
        # torch.save(checkpoint, '../model/omega_model/ckpt_%s.pth' % (str(epoch)))
        lr_scheduler.step()

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_model = model
            torch.save(checkpoint, '../model/omega_model/ckpt_best.pth')

        # print('----------CURRENT CKPT PREDICTION RESULT-----------')
        # current_epoch_checp = '../model/checkpoint/ckpt_%s.pth' % (str(epoch))
        # test(test_loader, current_epoch_checp)
        # print('----------------------------------------------------')
    '''

    print('----------PREDICTION-----------')
    test(test_loader,'../model/omega_model/ckpt_best.pth')
    # test(test_loader1, '../model/omega_model/ckpt_best.pth')

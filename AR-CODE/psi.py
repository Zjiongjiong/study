...
from data_input import train_loader,valid_loader
...
def test(data_loader,path):
    ...
    for i in range(0, len(b)):
        output_file_path = '../result/psi_pred/'
...
'''
path_checkpoint = "../model/psi_model/ckpt_best.pth"
...
if __name__ == "__main__":
    ...
    if RESUME:
        path_checkpoint = "../model/psi_model/ckpt_100.pth"  # 断点路径
    ...
    # ----------------------------------------tensorboard----------------------------------------
    loss_show = []
    train_writer = SummaryWriter('../result/psi_plot/loss/train')
    val_writer = SummaryWriter('../result/psi_plot/loss/val')
    # ----------------------------------------tensorboard----------------------------------------
    ...
    for epoch in range(start_epoch + 1, EPOCH+1):
        ...
        if not os.path.isdir("../model/psi_model"):
            os.mkdir("../model/psi_model")
        # torch.save(checkpoint, '../model/psi_model/ckpt_%s.pth' % (str(epoch)))
        lr_scheduler.step()

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_model = model
            torch.save(checkpoint, '../model/psi_model/ckpt_best.pth')
    ...
    '''
    print('----------PREDICTION-----------')
    test(test_loader,'../model/psi_model/ckpt_best.pth')
